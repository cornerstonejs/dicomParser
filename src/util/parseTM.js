...
dicomParser.parseTM = function(time, validate) {
  ...
  if (time.length >= 2) {
    ...
    var hh = parseInt(time.substring(0, 2), 10),
        mm = time.length >= 4 ? parseInt(time.substring(2, 4), 10) : undefined,
        ss = time.length >= 6 ? parseInt(time.substring(4, 6), 10) : undefined,
        ffffff = time.length >= 8 ? parseInt(time.substring(7, 13), 10) : undefined;
